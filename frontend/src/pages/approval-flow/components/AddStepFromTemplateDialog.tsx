...
interface AddStepFromTemplateDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  templates: ApprovalStepTemplate[];
  nextStepOrder: number;
  loading: boolean;
  onSubmit: (payload: CreateApprovalFlowStepFromTemplateRequest) => Promise<void>;
}

export function AddStepFromTemplateDialog({
  open,
  onOpenChange,
  templates,
  nextStepOrder,
  loading,
  onSubmit,
}: AddStepFromTemplateDialogProps) {
  const [templateId, setTemplateId] = useState("");
  const [stepOrder, setStepOrder] = useState(String(nextStepOrder));
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Thêm bước vào flow</DialogTitle>
          <DialogDescription>
            Chọn một mẫu bước duyệt đã tạo trong DB để thêm vào flow hiện tại.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4">
              ...
              <SelectContent>
                {templates.map((template) => (
                  <SelectItem key={template.id} value={String(template.id)}>
                    {template.stepName} - {getApproverTypeLabel(template.approverType)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2">
            <Label htmlFor="flow-step-order">Thứ tự trong flow</Label>
            <Input
              id="flow-step-order"
              type="number"
              min={1}
        ...
        <DialogFooter>
          ...
          >
            {loading ? "Đang thêm..." : "Thêm vào flow"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
